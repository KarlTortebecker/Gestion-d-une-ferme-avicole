...
public void run(String... args) throws Exception {
	...
	cm.setNomClient("Azangue");
	cm.setNomCmd("oeufs");
	cm.setPrenomClient("William");
	cm.setClientCmd(c);
	Pertes per= new Pertes();
	per.setDate(LocalDate.now());
	per.setOriginePerte("maladie");
	per.setQuantite(100);
	per.setTypePerte("suppression d'une bande");
	Produits o= new Produits();
	o.setDate(LocalDate.now());
	o.setNomProduit("oeufs");
